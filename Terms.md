---
layout: default
title: Terms of Use
---

# HAR Inspector Terms of Use

Effective date: 2026-05-08

These terms apply to HAR Inspector for macOS, developed and published by Bifang (Beijing) Technology Co., Ltd.

## License

Bifang (Beijing) Technology Co., Ltd. grants you a non-transferable license to use HAR Inspector according to the Mac App Store terms and these terms. Some features require HAR Inspector Pro.

## Your data and responsibilities

You are responsible for the HAR files and exports you open, analyze, store, sync, or share. HAR files may contain sensitive information such as cookies, authorization headers, API keys, tokens, personal data, and internal service URLs. Review and redact data before sharing.

## Pro features and purchases

Pro features are unlocked through Apple StoreKit purchases, trials, or other entitlements presented in the app. Purchases and refunds are handled by Apple under the applicable App Store policies.

## Acceptable use

Do not use HAR Inspector to access, process, export, or share data you are not authorized to handle. Do not use the app in a way that violates applicable laws, contracts, or platform rules.

## No warranty

HAR Inspector is provided "as is" without warranties of any kind. Diagnostics, security scans, formatters, and generated exports are best-effort tools and may be incomplete or inaccurate. You should verify important findings independently.

## Limitation of liability

To the maximum extent permitted by law, Bifang (Beijing) Technology Co., Ltd. is not liable for indirect, incidental, consequential, special, or punitive damages, or for loss of data, profits, or business opportunities arising from use of the app.

## Changes

These terms may be updated for new features, legal requirements, or operational changes. Continued use of the app after updates means you accept the updated terms.

## Contact

Bifang (Beijing) Technology Co., Ltd.

For support questions, contact: account1[at]kanatacode[.]com
